define(["require", "exports", "./m1-prototype", "./m1-prototype", "./m2-resources"], function (require, exports, m1_prototype_1, m1_prototype_2, m2_resources_1) {
    ...
    exports.timer = exports.accordion = exports.test = void 0;
    const test = (function () {
        let test = m1_prototype_1.objectCreator(m1_prototype_2.Operation.prototype);
        test.container = $("#test");
        test.name = "test";
        ...
        test.init = function () {
            ...
            this.modules = m2_resources_1.maths.settings.test.modules.split(",");
            this.times = m2_resources_1.maths.settings.test.times.split(",");
            this.exerciseNum = parseInt(m2_resources_1.maths.settings.test.exerciseNum);
            this.unlocked = parseInt(m2_resources_1.maths.settings.test.unlocked);
            choiceButtons.each(function (index) {
                if (index <= m2_resources_1.maths.test.unlocked)
            ...
            if (this.levelDisplayed === -1) {
                choiceButtons.on("click", function (e) {
                    m2_resources_1.maths.test.level = choiceButtons.index(this);
                    test.createTest();
                });
        ...
        test.createTest = function () {
            ...
            for (let i = 0; i < max; i += 1) {
                module = m2_resources_1.maths[this.modules[i].name];
                if (module) {
                    ...
                    html = m2_resources_1.maths.layout.main(this); // string containing markup for operations
                    if (i === max - 1) { // add bottom navigation depending on the position in test
                        html += m2_resources_1.maths.layout.testNavigation(false, true);
                    }
                    else {
                        html += m2_resources_1.maths.layout.testNavigation(false, false);
                    }
                    content[module.name] = html; // create property of content object and assign it the markup
                }
            }
            content["summary"] = this.summary;
            m2_resources_1.maths.accordion.init($("#test .content"), this.name, content);
            m2_resources_1.maths.handlers.test(this);
        };
        test.info = function () {
            let html = `<div class="test-interface">';
                    <h3 class="center">Level: ${m2_resources_1.maths.difficulties[this.level]}</h3>
            ...
            html += `<h3 class="center">Time to complete: ${this.times[this.level]} minutes</h3>
               ...
               ${m2_resources_1.maths.layout.testNavigation(true, false)}
              </div>`;
            return html;
        };
            ...
            const processResults = () => {
                ...
                if (score > 59 && this.unlocked < 2 && this.unlocked === this.level) {
                    if (score > 74) {
                        ...
                        m2_resources_1.maths.settings.accessStorage({ unlocked: this.level + 1 }, "maths.settings.test.", true);
                    }
                    else {
                ...
                ;
                m2_resources_1.maths.playSound(score > 59);
            };
            processResults();
            m2_resources_1.maths.accordion.unfold();
            m2_resources_1.maths.accordion.scrollTo(this.modules.length + 1);
        };
        return test;
    }());
    exports.test = test;
    const accordion = {
        ...
        attachListeners: function () {
            $('.test-accordeon-section-header').on("click", function () {
                if (!$(this).is(m2_resources_1.maths.accordion.last())) {
                    m2_resources_1.maths.accordion.show(this);
                }
            });
        },
