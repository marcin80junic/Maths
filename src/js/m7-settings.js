define(["require", "exports", "./m1-prototype", "./m2-resources"], function (require, exports, m1_prototype_1, m2_resources_1) {
    "use strict";
    ...
    const settings = {
        ...
        init: function () {
            let ns = m2_resources_1.maths.settings, namespace = "maths.settings.", all = {
                system: this.system,
            ...
            this.fields.volume.on('change', function () {
                ...
                m2_resources_1.maths.playSound(true, volume);
                enableApplyButton();
            });
        ...
        applyChanges: function () {
            let changed = this.changed, apply = false, temp, isEmpty, resetModules = () => {
                for (const object in m2_resources_1.maths) { // the program to reload modules next time they will be displayed
                    if (m2_resources_1.maths.hasOwnProperty(object) &&
                        Object.getPrototypeOf(m2_resources_1.maths[object]) === m1_prototype_1.Operation.prototype) {
                        m2_resources_1.maths[object]["levelDisplayed"] = -1; // actual reset
                    }
                }
            }, resetTest = () => {
                m2_resources_1.maths.settings.test.unlocked = "0";
        ...
        updateVolumeLabel: function (value) {
            ...
            if (value < 1)
                $('#speaker').prop('src', m2_resources_1.maths.icons.volumeMuted);
            else if (value < 30)
                $('#speaker').prop('src', m2_resources_1.maths.icons.volumeLow);
            else if (value < 70)
                $('#speaker').prop('src', m2_resources_1.maths.icons.volumeMedium);
            else
                $('#speaker').prop('src', m2_resources_1.maths.icons.volumeHigh);
        }
    };
