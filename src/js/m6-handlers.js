    ...
    const handlers = {
        exercises: function (module) {
            ...
            const goodAnswer = function () {
                ...
                icon.prop("src", m2_resources_1.maths.icons.tick);
                checkBtn.addClass("invisible");
                return true;
            }, wrongAnswer = function () {
                icon.prop("src", m2_resources_1.maths.icons.cross);
                ...
                answers = [];
                let isCorrect = m2_resources_1.maths.handlers.validateOperation(answerField, result, answers);
            ...
            levelChoice.on("change", function () {
                ...
                module.setLevel(m2_resources_1.maths.difficulties.indexOf(name));
            });
                    ...
                    if (e.type === "mouseover") { // position tooltip by setting its margin-left property
                        ...
                        timeout = setTimeout(function () {
                            if (tip.children().length === 0) {
                                m2_resources_1.maths.createAndAppendCanvas(tip, module, idx); // create tiptext content
            ...
            checkButtons.on("click", function (e) {
                ...
                m2_resources_1.maths.playSound(isCorrect);
                $(this).blur();
            });
            resetButton.on("click", () => {
                ...
                icons.prop("src", m2_resources_1.maths.icons.questMark);
        ...
        test: function (module) {
            let rows = module.container.find(".columns-line"), answerFields = module.container.find(".answer"), prevBtn = module.container.find('.button-prev'), nextBtn = module.container.find('.button-next'), startBtn = module.container.find('.button-start'), finishBtn = module.container.find('.button-finish'), headers = module.container.find('.test-accordeon-section-header'), closeBtn = m2_resources_1.maths.accordion.closeBtn, closeBtn2 = $("#test-close"), lastFocusableElements = $(startBtn).add(nextBtn).add(finishBtn).add(closeBtn).add(closeBtn2);
            const processTest = function () {
                ...
                const wrongAnswer = function (index) {
                    ...
                    icons.eq(index).prop("src", m2_resources_1.maths.icons.cross);
                };
                const correctAnswer = function (index) {
                    scores[index] = 1;
                    rows.eq(index).find(".answer").addClass("correct");
                    icons.eq(index).prop("src", m2_resources_1.maths.icons.tick);
                };
                rows.each(function (idx, row) {
                    let fields = $(row).find(".answer"), result = module.results[idx], isCorrect = m2_resources_1.maths.handlers.validateOperation(fields, result);
                    if (isCorrect) {
                        correctAnswer(idx);
                    }
                    else {
            ...
            startBtn.on("click", function () {
                if ($(this).text() === "Start") {
                    let ns = m2_resources_1.maths.test;
                    m2_resources_1.maths.timer.init($('.test-accordeon-titlebar-foo'), ns.times[ns.level], ns.summary);
                    m2_resources_1.maths.accordion.attachListeners();
                    $(this).text("Next");
                }
                m2_resources_1.maths.accordion.show(headers.eq(1), 1);
            });
            prevBtn.on("click", function () {
                let index = prevBtn.index(this);
                m2_resources_1.maths.accordion.show(headers.eq(index), index);
            });
            nextBtn.on("click", function () {
                let index = nextBtn.index(this);
                m2_resources_1.maths.accordion.show(headers.eq(index + 2), index + 2);
            });
            finishBtn.on("click", function () {
                let scores = processTest();
                answerFields.prop("disabled", "true");
                $(startBtn).add(prevBtn).add(nextBtn).add(finishBtn).hide();
                m2_resources_1.maths.test.displayResults(scores, m2_resources_1.maths.timer.stop());
            });
            closeBtn2.on("click", () => m2_resources_1.maths.accordion.dispose());
        ...
        textInputs: function (rows, answerFields, callback) {
            ...
            answerFields.on("blur", function () {
                ...
                if (field.val() === "") {
                    ...
                    parent.find(".icon").prop("src", m2_resources_1.maths.icons.questMark);
                }
            });
