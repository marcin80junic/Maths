$(function () {
    ...
    //adjust padding if window resizes*/
    $(window).resize(() => {
...
maths.fractions = (function () {
    ...
    fractions.getNumbers = function (level) {
        const rand = maths.range, fraction = (base, max) => [rand(1, max), base];
...
maths.test = (function () {
    ...
    test.createTest = function () {
        ...
        for (let i = 0; i < max; i += 1) {
            module = maths[this.modules[i].name];
            if (module) {
...
maths.layout = {
    ...
    main: function (module) {
        let isTest = module.name === "test", isRandomized = (maths.settings.general.isRandomized === "true"), isFraction, isAnswer, sign = module.sign, allNumbers = module.numbers, // numbers bank
...
maths.handlers = {
    ...
    test: function (module) {
        let rows = module.container.find(".columns-line"), answerFields = module.container.find(".answer"), prevBtn = module.container.find('.button-prev'), nextBtn = module.container.find('.button-next'), startBtn = module.container.find('.button-start'), finishBtn = module.container.find('.button-finish'), headers = module.container.find('.test-accordeon-section-header'), closeBtn = maths.accordion.closeBtn, closeBtn2 = $("#test-close"), lastFocusableElements = $(startBtn).add(nextBtn).add(finishBtn).add(closeBtn).add(closeBtn2);
        const processTest = function () {
            ...
            rows.each(function (idx, row) {
                let fields = $(row).find(".answer"), result = module.results[idx], isCorrect = maths.handlers.validateOperation(fields, result);
                if (isCorrect) {
                    correctAnswer(idx);
                }
                else {
...
maths.settings = {
    ...
    init: function () {
        let ns = maths.settings, namespace = "maths.settings.", all = {
            system: this.system,
        ...
        let changed = this.changed, apply = false, temp, isEmpty, resetModules = () => {
            for (const object in maths) { // the program to reload modules next time they will be displayed
                if (maths.hasOwnProperty(object) &&
                    ...
                    maths[object]["levelDisplayed"] = -1; // actual reset
                }
            }
        }, resetTest = () => {
